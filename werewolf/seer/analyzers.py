...
class VotingPatternAnalyzer(BaseAnalyzer):
    ...
    def _do_analyze(self, player_name: str, context: Dict[str, Any]) -> str:
        """
        分析投票模式
        ...
        if not isinstance(vote_history, list) or len(vote_history) < 2:
            return 'unknown'
        
        accurate_votes = sum(1 for vote in vote_history if vote.get('target_was_wolf'))
        total_votes = len(vote_history)
        accuracy_rate = accurate_votes / total_votes if total_votes > 0 else 0.5
    ...
    def analyze_pattern(self, player_name: str, context: Dict[str, Any]) -> str:
        """公共接口：分析投票模式"""
...
class GamePhaseAnalyzer(BaseAnalyzer):
    ...
    def _do_analyze(self, day: int, alive_count: int) -> GamePhase:
        """
        评估当前游戏阶段
        ...
        Returns:
            游戏阶段
        """
        if alive_count <= self.config.ENDGAME_ALIVE_THRESHOLD:
            return GamePhase.CRITICAL
        elif day <= self.config.EARLY_GAME_MAX_DAY:
            return GamePhase.EARLY
        elif day <= self.config.MID_GAME_MAX_DAY:
            return GamePhase.MID
        else:
            return GamePhase.LATE
    
    def _get_default_result(self) -> GamePhase:
        return GamePhase.EARLY
    
    def get_current_day(self, night_count: int) -> int:
        """获取当前天数"""
        return night_count
    
    def get_phase(self, day: int, alive_count: int) -> GamePhase:
        """获取游戏阶段"""
        return self.analyze(day, alive_count)
    
    def get_strategy(self, phase: GamePhase, alive_count: int) -> str:
        """获取阶段策略指导"""
        if phase == GamePhase.CRITICAL:
            return f"""ENDGAME ({alive_count} alive): CRITICAL PHASE
- MUST reveal identity and share all check results
- Every vote is critical - guide voting decisively
- Share complete check history in every speech
- No time for subtlety - be direct and authoritative"""
        
        elif phase == GamePhase.LATE:
            return """LATE GAME (Day 6+): FULL REVEAL RECOMMENDED
- Strongly consider revealing identity if not already
- Share all check results to guide good faction
- Lead voting with authority
- Wolves are desperate - watch for aggressive plays
- Correct all misinformation immediately"""
        
        elif phase == GamePhase.MID:
            return """MID GAME (Day 3-5): STRATEGIC REVEAL MODE
- Reveal if you have wolf check (guide voting)
- Reveal if fake seer appeared (counter-claim)
- Balance between staying hidden and building trust
- Analyze voting patterns - wolves show patterns now
- Correct injection attacks publicly"""
        
        else:  # EARLY
            return """EARLY GAME (Day 1-2): OBSERVATION MODE
- Stay hidden unless you have wolf check
- Gather information through observation
- Build trust scores through analysis
- Watch for injection attacks and false quotes
- Hint at suspicions without revealing role"""
